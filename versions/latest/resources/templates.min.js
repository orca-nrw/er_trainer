import{html,render}from"./../libs/lit/lit.js";export{render};export function main(app,data,events,phrase_nr,show_solution){const notation=app.notations[data.notation];const phrase=data.phrases[phrase_nr-1];const section=data.sections[phrase_nr-1];const heading=section.correct===undefined?"heading":section.correct?"correct":"failed";const is_binary=phrase.entities.length===2;return html`
  ...
  `;function diagram(is_solution){const is_recursive=is_binary&&phrase.entities[0]===phrase.entities[1]&&!notation.left;return html`
    ...
    `;function connection(nr){return nr?html`
      ...
      `:html`<div></div>`}function entity(nr){const check=nr=>section.input[nr-1]===phrase.solution[nr-1];return phrase.entities[nr-1]?html`
      ...
      `;return html`<div class="line${nr}"></div>`}}function select(nr){return html`
      <div class="col m-2">
    ...
    `}}export function legend(app){return html`
